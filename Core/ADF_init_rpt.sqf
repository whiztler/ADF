...
if !(isNil "ADF_HC1") then {_ADF_log_HC = _ADF_log_HC + 1};
if !(isNil "ADF_HC2") then {_ADF_log_HC = _ADF_log_HC + 1};
if !(isNil "ADF_HC3") then {_ADF_log_HC = _ADF_log_HC + 1};

_ADF_log_players	= (count allPlayers) - _ADF_log_HC;
_ADF_log_ai		= (count allUnits) - (_ADF_log_players + _ADF_log_HC);
if ((_ADF_log_ai - _ADF_log_players) < 0) then {_ADF_log_ai = 0};
...
if (!ADF_debug && ADF_Log_ServerPerfEnable) then { // ADF_debug already reports on server FPS
	[] spawn {
		waitUntil {
			...
			_ADF_log_players	= {alive _x} count allPlayers;
