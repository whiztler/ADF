...
{		
	...
	{
		_scr = _this execVM "Core\TP\DAC\Scripts\DAC_Check_ObjScr.sqf";
		...
		if(DAC_Basic_Value != 10) then
		{
			...
			if((_wpc + _DAC_WP_Typ) == 0) then
			{
				...
				[(_this select 0),_DAC_Count_WP,_obj_Nr,_inst,_objArray,(_this select 7)] execVM "Core\TP\DAC\Scripts\DAC_SimplePos_Objects.sqf";
			}
			else
			{
				if(count _objArray > 1) then
				...
				if(count _this == 8) then
				{
					if(_inst > 0) then
					{
						...
						{
							[(_this select 0),(_DAC_Count_WP - 0.5),_wpc,_DAC_WP_Typ,_obj_Nr,_inst,_objArray,(_this select 7),_dist,objNull,_logA] execVM "Core\TP\DAC\Scripts\DAC_Create_Objects.sqf";
						}
						else
						{
							[(_this select 0),_DAC_Count_WP,_wpc,_DAC_WP_Typ,_obj_Nr,_inst,_objArray,(_this select 7),_dist,objNull,_logA] execVM "Core\TP\DAC\Scripts\DAC_Create_Objects.sqf";
						};
					...
					switch (_run) do
					{
						case 3:	{
									...
									else
									{
										...
										{
											[_KiZone,10,_DAC_ZoneRadiusX,_DAC_ZoneRadiusY,0] execVM "Core\TP\DAC\Marker\DAC_ZoneMarker.sqf";
										}
										else
										{
											[_KiZone,10,0,_DAC_PolyCount,0] execVM "Core\TP\DAC\Marker\DAC_ZoneMarker.sqf";
						...
						case 12:{
									...
									if((DAC_Marker_Val select 1) == 1) then {if(count _MTemp > 0) then {[_MTemp]execVM "Core\TP\DAC\Marker\DAC_DeleteMarker.sqf"}};
