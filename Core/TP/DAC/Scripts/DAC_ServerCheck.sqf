...
if(local player) then
{
	...
	{
		if((DAC_Com_Values select 3) == format["%1",(name player)]) then {[0] execVM "Core\TP\DAC\Scripts\DAC_Show_Data.sqf"};
	}
	else
	{
		if((DAC_Com_Values select 3) == 1) then {[0] execVM "Core\TP\DAC\Scripts\DAC_Show_Data.sqf"};
	};
}
else
{
	if((typename (DAC_Com_Values select 3)) == "STRING") then
	{
		[0] execVM "Core\TP\DAC\Scripts\DAC_Show_Data.sqf";
	}
	else
	{
		if((DAC_Com_Values select 3) == 1) then {[0] execVM "Core\TP\DAC\Scripts\DAC_Show_Data.sqf"};
