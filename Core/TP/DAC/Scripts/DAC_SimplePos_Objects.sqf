...
if(isServer) then
{
	...
	if(count _this == 6) then
	{
		if(_inst > 0) then
		{
			...
			{
				[(_this select 0),(_DAC_Count_WP - 0.5),_obj_Nr,_inst,_objArray,(_this select 5),_dist,_logA] execVM "Core\TP\DAC\Scripts\DAC_SimplePos_Objects.sqf";
			}
			else
			{
				[(_this select 0),_DAC_Count_WP,_obj_Nr,_inst,_objArray,(_this select 5),_dist,_logA] execVM "Core\TP\DAC\Scripts\DAC_SimplePos_Objects.sqf";
			};
