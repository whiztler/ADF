...
while {_run > 0} do
{
	switch (_run) do
	{
		...
		case 3:	{
					...
					if(_DAC_WP_Typ == 1) then {{[_x]execVM "Core\TP\DAC\Scripts\DAC_Init_Waypoint.sqf"} foreach _ScriptArray};
		...
		case 13:{
					...
					{deletevehicle _x} foreach [_DumyA,_DumyB,_Null];
					if(count _MTemp > 0) then {[_MTemp]execVM "Core\TP\DAC\Marker\DAC_DeleteMarker.sqf"};
					if(!(_DAC_WP_Typ == 1)) then
					{
						_run = 0;
					...
					else
					{
						...
						{
							[1]execVM "Core\TP\DAC\Scripts\DAC_Create_Groups.sqf";
							execVM "Core\TP\DAC\Marker\DAC_LinkMarker.sqf";
