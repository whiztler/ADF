...
if(_mode == 1) then
{
	...
	{
		if((DAC_Com_Values select 3) == format["%1",(name player)]) then {[1] execVM "Core\TP\DAC\Scripts\DAC_Show_Data.sqf"};
	}
	else
	{
		if((DAC_Com_Values select 3) == 1) then {[1] execVM "Core\TP\DAC\Scripts\DAC_Show_Data.sqf"};
	};
};
