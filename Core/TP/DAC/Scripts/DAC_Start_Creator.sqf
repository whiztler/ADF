...
if(DAC_Code < 2) then
{
	[] execVM "Core\TP\DAC\Scripts\DAC_Init_Creator.sqf";
	[] execVM "Core\TP\DAC\Scripts\DAC_Vehicle_check.sqf";
	DAC_Dyn_Weather execVM "Core\TP\DAC\Scripts\DAC_Dyn_Weather.sqf";if(DAC_Direct_Start) then {DAC_Basic_Value = 1};
	...
	else
	{
		...
		if(DAC_Player_Marker) then {if(local player) then {[player] execVM "Core\TP\DAC\Marker\DAC_PlayerMarker.sqf"}};
		if((local player) && (DAC_Join_Action)) then {[] execVM "Core\TP\DAC\AI_1\DAC_Check_Action.sqf"};
		if(DAC_Marker > 0) then {[]execVM "Core\TP\DAC\Marker\DAC_InitMarker.sqf"};
	};
}
else
{
	...
	DAC_Dyn_Weather execVM "Core\TP\DAC\Scripts\DAC_Dyn_Weather.sqf";
	if(DAC_Player_Marker) then {if(local player) then {[player] execVM "Core\TP\DAC\Marker\DAC_PlayerMarker.sqf"}};
	_sum = time;
	sleep 3;
	if(DAC_Code == 3) then
	{
		...
		if(DAC_Marker > 0) then {if(local player) then {[player,1,(group player)] execVM "Core\TP\DAC\Marker\DAC_GroupMarker.sqf";[]execVM "Core\TP\DAC\Marker\DAC_InitMarker.sqf"}};
		[1] spawn DAC_fClientCheck;
		[] spawn DAC_fClientRadio;
		if(DAC_Join_Action) then {[] execVM "Core\TP\DAC\AI_1\DAC_Check_Action.sqf"};
	}
	else
	{
		if((DAC_Com_Values select 1) > 0) then
