...
_scr = _para execVM "Core\TP\DAC\Scripts\DAC_Insert_Zone.sqf";
waituntil {scriptdone _scr};
_scr = [1,_name]execVM "Core\TP\DAC\Scripts\DAC_Insert_Waypoint.sqf";
waituntil {(DAC_NewZone != 1)};
if(DAC_NewZone == 2) then
{
	_scr = [1,_name]execVM "Core\TP\DAC\Scripts\DAC_New_Groups.sqf";
	waituntil {scriptdone _scr};
	DAC_NewZone = 0;
